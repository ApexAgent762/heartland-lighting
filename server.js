...
cloudinary.config({ cloud_name: 'dzh3iu4ea', api_key: '977445886732386', api_secret: 'jbgKNUq-Cflnac5SrhZ42x318ZA' });
...
function loadData() {
  if (!fs.existsSync(DATA_FILE)) {
    const initial = {
      users: [
        { id: 1, username: 'lane', password: bcrypt.hashSync('lane2026', 10), role: 'owner', name: 'Lane Tietz' },
        ...
        { id: 4, username: 'ty', password: bcrypt.hashSync('ty2026', 10), role: 'employee', name: 'Ty Rogers', rate: 40, color: '#EAF3DE', tc: '#27500A' },
...
app.get('/api/my-stats', auth(['employee']), (req, res) => {
  ...
  res.json({ id: user.id, name: user.name, rate: user.rate, color: user.color, tc: user.tc, revenue, installs: mySales.length, earned: revenue * (user.rate / 100), sales: mySales.slice(-10).reverse(), goal: data.goals.find(g => g.userId === req.user.id) || null, posts: data.posts.slice(0, 10) });
});
...
app.get('/api/my-rank', auth(['employee']), (req, res) => {
  ...
  const rank = employees.findIndex(e => e.id === req.user.id) + 1;
  res.json({ rank, total: employees.length });
});

app.post('/api/sales', auth(['secretary', 'owner', 'employee']), (req, res) => {
...
app.get('/api/owner-sales', auth(['owner']), (req, res) => {
  const data = loadData();
  const mySales = data.sales.filter(s => s.userId === req.user.id);
  const revenue = mySales.reduce((s, x) => s + x.amount, 0);
  res.json({ revenue, installs: mySales.length, sales: mySales.slice(-20).reverse() });
});
...
app.get('/api/quotes', auth(['owner', 'secretary']), (req, res) => { res.json(loadData().quotes || []); });
app.post('/api/quotes', auth(['employee', 'owner']), (req, res) => {
  const data = loadData();
  if (!data.quotes) data.quotes = [];
  const d = new Date();
  data.quotes.unshift({ id: Date.now(), userId: req.user.id, employeeName: req.user.name, address: req.body.address, building: req.body.building, footage: req.body.footage, price: req.body.price, discount: req.body.discount || 0, finalPrice: req.body.finalPrice, status: 'new', notes: '', date: d.toISOString().split('T')[0] });
  saveData(data); res.json({ ok: true });
});
...
app.get('/api/summary', auth(['owner', 'secretary']), (req, res) => {
  const data = loadData();
  const now = new Date();
  ...
  const employees = data.users.filter(u => u.role === 'employee').map(u => {
    const revenue = data.sales.filter(s => s.userId === u.id).reduce((s, x) => s + x.amount, 0);
    return { id: u.id, name: u.name, revenue };
  });
  const topPerformer = [...employees].sort((a, b) => b.revenue - a.revenue)[0];
  res.json({
    weekRev: weekSales.reduce((s, x) => s + x.amount, 0),
    todayRev: todaySales.reduce((s, x) => s + x.amount, 0),
...
app.post('/api/photos', auth(['employee', 'owner', 'secretary']), async (req, res) => {
  try {
    const { image, address } = req.body;
    const result = await cloudinary.uploader.upload(image, { folder: 'heartland', transformation: [{ quality: 'auto', fetch_format: 'auto' }] });
    const data = loadData();
    if (!data.photos) data.photos = [];
    data.photos.unshift({ id: Date.now(), userId: req.user.id, employeeName: req.user.name, address: address || '', url: result.secure_url, date: new Date().toISOString().split('T')[0] });
    saveData(data);
...
app.get('/api/photos', auth(), (req, res) => {
  ...
  if (req.user.role === 'employee') return res.json(photos.filter(p => p.userId === req.user.id));
  res.json(photos);
});
